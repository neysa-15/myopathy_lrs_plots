...
`/path/to/mt_dna_coverage.txt` containing:
* `Sample`: sample name
...
`/path/to/all_STR_loci.all_cohorts.tsv` containing these columns:
* `GENE`
* `SAMPLE_ID`
* `ALLELE_ID`
* `COHORT`

`/path/to/all_STR_loci.all_cohorts.copy_numbers.tsv` containing these columns:
* `GENE`
* `ALLELE_ID`
* `COPIES`
* `COPIES NORMALISED`
...
### sup table 3
`/path/to/all_svlog_database.txt` containing `*/database_annotation.tsv` file for each sample

`/path/to/sniffles_overlapping_panel.tsv` containing lines with these columns:
* `sample_id`
* `CHROM`
* `START`
* `END`
...
`/path/to/all_sv_annotations.txt` containing `*.hg38.svs.annotated.tsv` file for each sample

 ## mtDNA deletion analysis for fig 5i
